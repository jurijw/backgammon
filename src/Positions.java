...
public class Positions {
     ...
     */
    public static final int MAX_PIECES_PER_BOARD_POSITION = 5;
    /**
     ...
     * The start index for the black end zone.
     */
    public static final int END_ZONE_END_INDEX_WHITE = 23;
     ...
     * pieces.
     */
    public static final int[] DEFAULT_POSITION_SETUP = {2, 0, 0, 0, 0, -5, 0, -3, 0, 0, 0, 5,
     ...
     */
    boolean validIndex(int... indices) {
     ...
     * captured pieces.
     */
    boolean validBoardIndex(int... indices) {
        for (int index : indices) {
    ...
    public int numEscaped(boolean white) {
        if (white) {
            return get(WHITE_CAPTURED_INDEX);
        } else {
            return get(BLACK_CAPTURED_INDEX);
        }
    }

    /** Returns the number of captured pieces of the player specified by WHITE. */
    public int numCaptured(boolean white) {
        return white ? get(WHITE_CAPTURED_INDEX) : get(BLACK_CAPTURED_INDEX);
    }
     ...
     **/
    boolean isLastPieceOnBoard(int index, boolean white) {
        for (int position : occupiedBoardPositions(white)) {
            if (position > index) {
                return false;
            }
    ...
    boolean allEscaped(boolean white) {
        return numEscaped(white) == NUM_PIECES;
    }
